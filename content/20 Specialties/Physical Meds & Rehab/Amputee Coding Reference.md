...
- **Failing to capture laterality** — **[[Z89.519]]** (**unspecified**) is almost never appropriate when the chart clearly documents right or left. Always code to the highest specificity.

- **Missing the level** — "**Amputation, lower extremity**" without specifying above knee vs. below knee vs. foot is incomplete. Query the physician.

- **Not capturing bilateral amputations** — If the patient has had amputations on both sides at different levels (**common in diabetics**), code *all* applicable **Z89** codes. Each limb/level gets its own code.

- **Forgetting the underlying etiology** — After the **Z89** status codes, coders sometimes stop. Always code the underlying condition (**DM, PVD, trauma, malignancy**) as an additional diagnosis. This supports medical necessity and carries HCC weight.

- **Phantom limb pain vs. phantom limb sensation** — **[[G54.6]]** (**with pain**) vs. [[G54.7]] (**without pain**). Do not assign pain unless the physician explicitly documents it. Query if ambiguous.

- **Modifier AI omission** — If the PMR physician is the *admitting/principal* physician, AI must be appended to the **E/M** code for Medicare billing. Omitting AI can result in claim denial or payment recovery.

- **Using the acute injury S code without the correct 7th character** — During a rehab admission, traumatic amputations should use the "S" (**sequela**) 7th character, not "A" (**initial**) or "D" (**subsequent encounter during healing**). The "S" signals that the patient is in the chronic/residual phase.

- **Billing therapeutic procedure codes without physician-performed documentation** — **[[97110]]**, **[[97116]]**, **[[97761]]** etc. require the *physician* to be the one performing the service. If the PT performs it, it's the PT's claim, not the PMR physician's.

- **Not querying for MRSA/organism specification** — When residual limb infection (T87.4x) is documented, always check whether a causative organism was identified and documented. If **MRSA**, add **[[B95.62]]**. If **Staph aureus** (**not MRSA**), add **[[B95.61]]**. Organism codes add clinical specificity.

- **Using T87 codes without a paired Z89 code** — T87 tells you *what complication*, but Z89 tells you *which limb*. Always pair them.
