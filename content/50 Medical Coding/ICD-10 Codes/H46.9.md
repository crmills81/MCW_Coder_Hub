...
- **May use with G35.A (MS) or G36.0 (NMO) if those conditions documented as underlying cause**
...
- **Multiple sclerosis (G35.A):** Some MS codes may map to HCC depending on model version
  - MS requires ongoing disease-modifying therapy (high cost)
  - Optic neuritis itself (H46.9) does not map to HCC
  - MS diagnosis (G35.A) may have HCC implications
...
- If MS diagnosed, code G35.A separately (may have HCC relevance)
...
- **Secondary diagnoses:**
  - G35.A (Multiple sclerosis) if known
...
| **G35.A** | **Multiple sclerosis** | **Most common associated condition** |
...
|-------------|-------------|-------------|
| G35.A | Multiple sclerosis | Primary associated disease |
...
│   │       ├── **Multiple sclerosis** → Add G35.A as comorbidity
...
- OR **MS-DRG 057** (Degenerative Nervous System Disorders with MCC) or **MS-DRG 058** (without MCC) if G35.A coded as principal
- Principal diagnosis selection determines DRG

**Rationale:**
Multiple sclerosis is the primary diagnosis (underlying disease). Optic papillitis is a manifestation of MS. Code MS (G35.A) as primary and optic neuritis (H46.02 or H46.9) as secondary. Specific type (papillitis) documented so H46.02 preferred over H46.9.
...
**ICD-10-CM Coding:**
- **G35.A** - Multiple sclerosis (**PRIMARY** - this is MS relapse)
...
**Rationale:**
In known MS patient with optic neuritis relapse, MS (G35.A) is primary diagnosis. Optic neuritis (H46.11) is secondary, representing acute manifestation of MS activity. Specific type (retrobulbar) documented so use H46.11 instead of H46.9.
...
- **If MS suspected/confirmed:** "Patient meets McDonald criteria for multiple sclerosis" → Code G35.A additionally
...
**ICD-10 Coding**: H46.9 (Unspecified optic neuritis) - may be updated to more specific code (H46.11 retrobulbar neuritis, right eye) after MRI confirmation. Will add G35.A.- (Multiple sclerosis) if MS criteria met based on MRI and clinical findings."
...
- Associated with multiple sclerosis (also code G35.A)?
...
4. **Not coding MS when diagnosed:**
   - If MS diagnosed, code G35.A as primary or comorbidity (depending on encounter reason)
