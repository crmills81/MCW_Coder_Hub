...
tags:
  ...
  - ICD-10-CM
code: E11.3491
description: Type 2 diabetes mellitus with severe nonproliferative diabetic retinopathy without macular edema, right eye
...
# 🧬ICD-10 CM E11.3491 - Type 2 diabetes mellitus with severe nonproliferative diabetic retinopathy without macular edema, right eye

## Overview
**ICD-10 CM [[E11.3491]]** is a specific combination code used to classify **Type 2 diabetes mellitus** complicated by **severe nonproliferative diabetic retinopathy (NPDR)** **without** **macular edema**, specifically localized to the **right eye**. This code captures the systemic disease, the specific ocular complication, the severity of the [[retinopathy]] (**severe**), the absence of **edema**, and the laterality in a single alphanumeric string.

Accurate coding of [[E11.3491]] is critical for **risk adjustment**, severity of illness scoring, and ensuring appropriate reimbursement. It distinguishes the condition from moderate NPDR ([[E11.3391]]) and **proliferative diabetic retinopathy** ([[E11.3511]]), as well as differentiating cases with macular edema ([[E11.3411]]) from those without.

## Code Breakdown
The structure of [[E11.3491]] follows the ICD-10-CM taxonomy:
...
| **Further Extension** | `E11.349` | Type 2 diabetes mellitus with severe nonproliferative diabetic retinopathy without macular edema |
...
> When documenting [[diabetic retinopathy]], the specific eye affected must be documented. If bilateral, use [[E11.3493]]. If unspecified, use [[E11.3499]], though unspecified codes should be avoided when clinical documentation supports specificity.
...
### Excludes
*   **Type 1 Diabetes:** If the patient has Type 1 diabetes, codes from category E10 must be used (e.g., [[E10.3491]]).
...
*   **Moderate Retinopathy:** If the [[retinopathy]] is moderate, codes from the `.33` series are required (e.g., [[E11.3391]]).
*   **Proliferative Retinopathy:** If the retinopathy is proliferative, codes from the `.35` series are required (e.g., [[E11.3591]]).
*   **Macular Edema:** If **macular edema** is present, codes from the `.341` series are required (e.g., [[E11.3411]]).

### Use of Additional Codes
While [[E11.3491]] is a combination code, additional codes may be required to fully capture the clinical picture:
...
In the inpatient setting, [[E11.3491]] influences the Medicare Severity Diagnosis Related Group (MS-DRG) assignment through Complication/Comorbidity (CC) logic.
...
**Scenario:** A patient with known [[Type 2 diabetes]] presents for a routine retina follow-up. Dilated exam reveals severe nonproliferative diabetic retinopathy without macular edema in the right eye. Left eye shows moderate NPDR without edema.
**Coding:**
*   Primary: [[E11.3491]] (Severe NPDR without ME, Right Eye)
...
**Scenario:** Patient has Type 2 diabetes. Exam shows severe NPDR without macular edema in both eyes.
**Coding:**
*   Primary: [[E11.3493]] (Do not use [[E11.3491]] and [[E11.3492]] together; use the bilateral code).

### Example 3: Development of Macular Edema
**Scenario:** Patient previously coded with [[E11.3491]] returns. Exam now shows severe nonproliferative diabetic retinopathy with macular edema in the right eye.
**Coding:**
*   Primary: [[E11.3411]] (Type 2 diabetes with severe nonproliferative diabetic retinopathy with macular edema, right eye).
*   *Note:* The code changes to reflect the development of edema.

### Example 4: Insulin Usage
**Scenario:** Patient with Type 2 diabetes coded as [[E11.3491]] is on long-term insulin therapy.
**Coding:**
*   Primary: [[E11.3491]]
...
*   **Denial Risk:** High if paired with incompatible procedure codes (e.g., anti-VEGF injection without documented macular edema).
