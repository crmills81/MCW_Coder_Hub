...
risk_adjustment_note: |
  This code does not map to a payment HCC category under CMS-HCC v28. As a Chapter 18 symptom code, it represents an active clinical finding rather than a definitive chronic disease burden. It does not independently drive RAF scores, but capturing it justifies the medical necessity for fall-risk interventions, physical therapy, and neurological diagnostic testing.
...
includes_terms:
  ...
  - Wobbliness on feet
ncci_notes: |
  In the profee/outpatient rehabilitation setting, if billing for physical therapy, NCCI edits often restrict billing 97112 (Neuromuscular re-education for balance) and 97116 (Gait training) during the same 15-minute time block. Distinct time intervals must be documented, and Modifier -59 may be required to bypass edits if both are performed in separate, distinct sessions on the same day.
