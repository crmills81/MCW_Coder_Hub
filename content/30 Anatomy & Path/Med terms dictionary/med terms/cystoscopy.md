...
>FROM #medterm 
>WHERE length(filter(roots, (word) => econtains([[cystoscopy]].roots, word))) > 0 AND file.name != [[cystoscopy]].file.name
>SORT file.name ASC
>```
_____
...
>TABLE definition AS Definition
>FROM #medterm 
>WHERE length(filter(definition, (word) => econtains([[cystoscopy]].definition, word))) > 0 AND file.name != [[cystoscopy]].file.name
>```
