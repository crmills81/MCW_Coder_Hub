...
alphabet: E
definition:
  - Inward turning or inversion of the eyelid margin, causing eyelashes to rub against the cornea
---
...
> | **[[H02.051]]** | Trichiasis without entropion, right upper eyelid *(if applicable)* |
> | **[[Q10.2]]** | Congenital entropion *(Excludes1 from H02.0- range)* |
>
> ## **CPT Codes**
>
> | Code | Description |
...
>FROM #medterm 
>WHERE length(filter(roots, (word) => econtains([[entropion]].roots, word))) > 0 AND file.name != [[entropion]].file.name
>SORT file.name ASC
>```
_____
...
>TABLE definition AS Definition
>FROM #medterm 
>WHERE length(filter(definition, (word) => econtains([[entropion]].definition, word))) > 0 AND file.name != [[entropion]].file.name
>```
